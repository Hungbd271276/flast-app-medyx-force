...
import './RecordHealthPage.css'
import { useParams } from 'react-router-dom';

const RecordHealthPage: React.FC = () => {
  ...
  const fetchServiceData = async (pageNumber: number, filters: any = {}, searchValue: string = '') => {
    try {
     ...
     if (userData?.id) {
      //   filterConditions = `MaBN:${userData.id}`;
      filterConditions = `MaBN:02000456`;
     }
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      if (isMounted) {
        await fetchServiceData(1, filterValues, searchTerm);
      }
    };
  ...
  const handlePageChange = async (pageNumber: number) => {
    ...
    await fetchServiceData(pageNumber, filterValues, searchTerm);
  };
  ...
  const handleClearFilter = async () => {
    setFilterValues({});
    setCurrentPage(1);
    await fetchServiceData(1, {}, searchTerm);
    setShowFilterPopover(false);
  };
  ...
  const handleSearch = async (value: string) => {
    ...
    const newTimeout = setTimeout(async () => {
      setSearchTerm(value);
      setCurrentPage(1);
      await fetchServiceData(1, filterValues, value);
    }, 1000);
    ...
    return (
      <div className="pagination-container">
        <div className="pagination-info">
          Hiển thị {((currentPage - 1) * pageSize) + 1} - {Math.min(currentPage * pageSize, totalRows)} của {totalRows} hồ sơ
        </div>
  ...
  const handleServiceClick = (record: any) => {
    history.push({
    pathname: '/health-profile-detail',
    state: { recordData: record }
  });
  };

  const renderServiceItem = (service: any, index: number) => {
    const visibleFields = service.patientData?.filter((field: any) => field.visible === true) || [];
    ...
    return (
      <IonCard key={index} className="service-list-item" onClick={() => handleServiceClick(service)}>
          ...
          <div className="service-item-main">
            ...
            <div className="service-item-info">
              <div className="service-item-title">
                {visibleFields.find((f: any) => f.mapfield === "TenGT")?.value || "Không rõ tên"}
              </div>
              <div className="service-item-details">
                {visibleFields.map((field: any, fieldIndex: number) => {
                  // Bỏ qua avatar vì đã hiển thị ở trên
                  if (field.mapfield === "avatar" || field.mapfield === "TenGT") return null;

                  return (
                    <div key={fieldIndex} className="service-item-field">
                      <span className="service-field-label">{field.key}:</span>
                      <span className="service-field-value">
                        {field.type === 'boolean'
  ...
  const renderContent = () => {
    ...
    if (currentData.length === 0) {
      return <EmptyState message="Không có dữ liệu dịch vụ" />;
    }
  ...
  return (
    <IonPage>
      <IonHeader className='ion-home-header_profile'>
        <IonToolbar className="tool-bar-header custom-toolbar">
          <div className="toolbar-flex">
            ...
            <div className="toolbar-center">Danh sách hồ sơ sức khỏe</div>
            <div className="toolbar-side" />
          </div>
...
export default RecordHealthPage;
